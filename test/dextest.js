...
contract.skip ("Dex", accounts => {
    it("The user must have ETH deposited such that deposited ETH >= buy order value", async () => {  
        ...
        await truffleAssert.reverts(
            dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 10000, 100)
        )

        dex.depositEth({value: 1000000}); 
        
        await truffleAssert.passes(
            dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 100, 100)
    ...
    it("The user must have enough tokens deposited such that token balance >= sell order amount", async () => {  
        ...
        await truffleAssert.reverts(
            dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 100, 100)
        )

        await link.approve(dex.address, 5000);
        await dex.addToken(link.address, web3.utils.fromUtf8("LINK"))
        await dex.deposit(web3.utils.fromUtf8("LINK"), 1000)
        await truffleAssert.passes(
            dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 10, 100)
        )
    ...
    it("The BUY order book should be ordered on price from highest to lowest starting at index 0", async () => {  
        ...
        dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 100, 300)
        dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 10, 200)
        dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 50, 400)
        // await dex.createLimitOrder(0, web3.utils.fromUtf8("LINK"), 400, 200)
        let orderbook = await dex.getOrderBook(web3.utils.fromUtf8("LINK"), 0)
        console.log("BUY ORDERBOOK: "+orderbook)
        for (let i = 0; i < orderbook.length - 1; i++) {
            assert(orderbook[i].price >= orderbook[i+1].price, "incorrect order of orderbook")
        }
    ...
    it("The SELL order book should be ordered on price from lowest to highest starting at index 0", async () => {  
        ...
        let link = await tokens.deployed()
        await link.approve(dex.address, 5000);
        await dex.deposit(web3.utils.fromUtf8("LINK"), 5000)
        dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 10, 30)
        dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 15, 10)
        dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 20, 20)
        let orderbook = await dex.getOrderBook(web3.utils.fromUtf8("LINK"), 1)
        console.log("SELL ORDERBOOK: "+orderbook)
         for (let i = 0; i < orderbook.length - 1; i++) {
        assert(orderbook[0].price <= orderbook[1].price, "incorrect order of order book")
         }
...
contract("Dex", accounts => {
    ...
    it("Market orders can be submitted even if the order book is empty", async () => {
        ...
        //difference between assert and truffle assert? 
